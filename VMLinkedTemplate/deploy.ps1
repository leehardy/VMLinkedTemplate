...
$templateFolder = "C:\Users\lee0592\Documents\personal-git-repos\VMLinkedTemplate\VMLinkedTemplate\VirtualMachines"
$templates = Get-ChildItem $templateFolder
$storageAccountName = "depltempsa1993"
...
function createStorageAccount ($resourceGroup) {
	Do {
		...
		$sa = Get-AzureRmStorageAccount -Name $saRandName -ResourceGroupName $resourceGroup
	} While ($sa)

	New-AzureRmStorageAccount -Name $sa -ResourceGroupName $resourceGroup -SkuName Standard_LRS -Location australiaeast -Kind Storage
}
...
Remove-AzureRmStorageAccount -Name $storageAccountName -ResourceGroupName $storageAccountResourceGroup
